...
bool VideoDriver::openDevice()
{
	...
	snprintf(file, sizeof(file), "/dev/video%d", m_cameraNum);
...
void VideoDriver::queryAvailableSizes()
{
	...
	while (xioctl(VIDIOC_ENUM_FRAMESIZES, &f) == 0) {
		if (f.un.discrete.width > 0 && f.un.discrete.height > 0)
			m_sizeList.append(QSize(f.un.discrete.width, f.un.discrete.height));
...
void VideoDriver::queryAvailableRates()
{
	...
	while (xioctl(VIDIOC_ENUM_FRAMEINTERVALS, &f) == 0) {
		if (f.un.discrete.numerator > 0 && f.un.discrete.denominator > 0)
			m_rateList.append(QSize(f.un.discrete.numerator, f.un.discrete.denominator));
